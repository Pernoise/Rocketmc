...
create_deb_package() {
    ...
    if [ -f "RocketMC.bin" ]; then
        read -p "RocketMC.bin already exists. Use existing binary? [Y/n]: " use_existing
        # default to yes if empty, and make case-insensitive
        use_existing=${use_existing:-Y}
        use_existing=$(echo "$use_existing" | tr '[:upper:]' '[:lower:]')
        if [ "$use_existing" != "y" ]; then
            echo -e "${YELLOW}Recreating RocketMC.bin...${NC}"
            ...
            echo -e "${GREEN}Using existing RocketMC.bin${NC}"
        fi
    else
        echo -e "${YELLOW}No existing RocketMC.bin found. Compiling...${NC}"
        create_venv
        if confirm_compile; then
            compile_application
        else
    ...
    cp RocketMC.bin compile-deb/usr/share/openlauncher/
    chmod +x compile-deb/usr/share/openlauncher/RocketMC.bin
    chmod -R 0755 compile-deb

    # Build .deb; use --root-owner-group to avoid owner/group warnings when running as root
    dpkg-deb --build --root-owner-group compile-deb "RocketMC.deb"
    ...
    if [ "$install_choice" == "y" ]; then
        ...
        sudo apt update
        sudo apt install -y ./RocketMC.deb
    fi

    rm compile-deb/usr/share/openlauncher/RocketMC.bin
}
